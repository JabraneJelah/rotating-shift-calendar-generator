...
| Production PWA         | `npm run test:e2e:pwa` |
...
PWA unit tests cover stable manifest construction, icon metadata and dimensions, request/query classification, exact message schemas, deterministic cache names and revisions, duplicate/conflicting assets, every hard count/size budget, timezone graph isolation, worker-template injection, atomic install/promotion, partial-activation rejection, two-generation cleanup, query-normalized root fallback, cached documents/static assets, unknown-route 503 fallback, request exclusions, client counts, and exact activation. Component tests cover connectivity announcements, install availability/dismissal, verified waiting updates, Later, unsafe-current-tab blocking, safe activation, and conservative unanswered-tab blocking. `test:e2e:pwa` uses a production server on an isolated origin and verifies registration/control, exact cache contents, headers, offline root and saved-planner restoration, offline edits/autosave/import/views/downloads, V1/malformed query behavior, cached content, cross-origin exclusion, privacy, a real release-A-to-B waiting-worker update with a second unsafe tab, planner preservation, cache cleanup, unknown-route fallback, and required responsive widths. Ordinary development and the default Playwright project do not register a worker.

Vitest uses isolated `vmThreads` with file parallelism disabled. This keeps the interaction-heavy 56-position editor test within its unchanged timeout on constrained Windows hosts without reducing assertions, extending timeouts, or sharing state between files.

Playwright runs the default development journeys serially so on-demand Next.js compilation and client navigation do not compete across browser workers on constrained hosts. The production PWA project is also serial because its release-A-to-release-B test deliberately replaces and restores the ignored generated worker artifact.

The production build must run twice from clean output when PWA graph logic changes. Compare the sorted URL/type/size/revision graph, timezone entry and referrer, count, raw/gzip/Brotli totals, largest asset, and generated worker. A changed Next.js build identifier may change HTML/chunk identities; every difference must be attributable to emitted build content rather than ordering, timestamps, absolute paths, locale, or host-only state.
