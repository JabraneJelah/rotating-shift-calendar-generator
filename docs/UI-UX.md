...
## Containment and spacing conventions

Phase 7B Step 4 established a border-first containment convention across the generator and its panels, replacing the floating-card shadow the main generator card previously carried. Borders (`border-border`) are the default way a panel identifies its own boundary; a soft shadow is reserved for the one surface in the app marked `role="dialog"` (the timed-export panel) and is not applied to static settings or status containers, however prominent. Two radius tiers are in active use and should be matched rather than re-derived: `rounded-2xl` for primary, top-level containers (the generator card, the Local Planners panel, the Shift details disclosure, the PWA status panel), and `rounded-xl` for secondary panels nested within a card (the date-exception editor, stat tiles, per-month year-view tiles). Spacing follows Tailwind's default 4px-based scale without a separate custom scale: container padding is `p-3`/`p-4` (with the single generator card's own `p-4 sm:p-7 lg:p-9` progression as the one deliberately larger exception, being the app's single largest container), gaps are `gap-2` through `gap-5`, and vertical rhythm between stacked elements runs `mt-1` through `mt-6`/`mt-8`. New spacing values should fit one of these existing steps rather than introduce a new one. Heading sizes inside the generator are capped at the same flat size as the calendar's own result heading (`text-2xl`, no responsive escalation) — a heading that introduces a result must not out-scale the result itself, the same principle the homepage hero already follows relative to that same result heading.
