...
The current Phase 6B1 product supports six verified presets: fixed `4 on / 4 off`, fixed `2-2-3`, fixed `7 on / 7 off`, rotating `2 Day / 2 Night / 4 Off`, the exact 28-day DuPont rotation, and rotating `7 Day / 7 Off / 7 Night / 7 Off`. It also supports custom cycles, month and year navigation, Monday- or Sunday-first calendars, next-schedule insights, worked-weekend-date totals, restorable versioned URLs, explicit link copying, monthly and full-year ICS downloads, and native printing of the active calendar view. Optional Shift details add private Day/Night labels and nominal hours. A focused one-date editor can privately replace a generated shift, mark Leave, Sick, or Training, add one additional-work occurrence, and attach one plain-text note. One effective projection drives month/year calendars, next-work information, weekend counts, nominal personal statistics, print, and exception-aware all-day ICS; private notes never enter files. An additional explicit timed export resolves work through pinned IANA 2026d data after the user selects a timezone. It is deliberately limited to occurrence starts from 1970 through 2037; all-day export remains available outside that range. A visitor may explicitly save up to 20 complete planners in this browser, reopen the last saved planner from a clean `/`, and export or atomically import readable JSON backups. Saved data is not synchronized or encrypted and may be removed with browser site data. V1 links remain base-only and exclude private planner state. Schedule calculations, persistence, sharing, file generation, and printing preparation happen locally in the browser.
